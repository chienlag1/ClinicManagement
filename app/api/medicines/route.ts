...
export async function POST(req: Request) {
  try {
    ...
    if (!body.medicine_code || !body.medicine_name || !body.type || !body.price || !body.unit) {
      return NextResponse.json(
        { error: 'Missing required fields: medicine_code, medicine_name, type, price, unit' },
        { status: 400 }
      );
    }

    // Validate string fields are not empty
    if (typeof body.medicine_code !== 'string' || body.medicine_code.trim() === '') {
      return NextResponse.json(
    ...
    if (typeof body.medicine_name !== 'string' || body.medicine_name.trim() === '') {
      return NextResponse.json(
