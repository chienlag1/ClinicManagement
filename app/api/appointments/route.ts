...
export async function POST(request: Request) {
  try {
    ...
    console.log('Parsed data:', { isNewPatient, selectedPatientId, clinic_id, doctor_id, appointment_date, appointment_time });

    // Validation
    if (!clinic_id || !doctor_id || !appointment_date || !appointment_time) {
      console.log('Missing required fields');
      return NextResponse.json({ 
        error: 'Thiếu thông tin bắt buộc: phòng khám, bác sĩ, ngày và giờ hẹn.' 
      }, { status: 400 });
    }
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: 'Đã có lỗi xảy ra khi lấy danh sách lịch hẹn' }, { status: 500 });
  }
}